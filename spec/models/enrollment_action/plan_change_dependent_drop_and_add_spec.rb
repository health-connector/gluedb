...
describe EnrollmentAction::DependentDropAndAdd, "given a qualified enrollment set, being persisted", :dbclean => :after_each do 
  ...
  let(:plan_cv) { instance_double(Openhbx::Cv2::PlanLink, :id => plan_2.hios_plan_id, :alias_ids => alias_ids) }
  let(:rating_area) { nil }
  let(:alias_ids) { nil }
  ...
  let(:new_policy_cv) { instance_double(Openhbx::Cv2::Policy, :id => new_eg_id, :enrollees => [enrollee_primary, enrollee_secondary, enrollee_new], :policy_enrollment => new_policy_enrollment, :previous_policy_id => nil) }
  let(:carrier) { instance_double(Carrier, :id => 1) }
