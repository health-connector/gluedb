...
- the second enrollment has some of the same members as the first enrollment and some different members, but the same subscriber" do
  ...
  let(:event_3) { instance_double(ExternalEvents::EnrollmentEventNotification, :existing_plan => different_plan, :all_member_ids => member_ids_2) }
  let(:event_set) { [event_1, event_2] }
  let(:non_qualifying_event_set) { [event_1, event_3] } # Dropped member on exisiting plan and added another member with Different plan
  ...
  it "does not qualify because it has different plans" do
    expect(subject.qualifies?(non_qualifying_event_set)).to be_falsey
  end
end
...
describe EnrollmentAction::DependentDropAndAdd, "given a qualified enrollment set, being persisted" do 
  ...
  let(:shop_market) { instance_double(::Openhbx::Cv2::PolicyEnrollmentShopMarket, :employer_link => employer_link, :total_employer_responsible_amount => total_employer_responsible_amount, :composite_rating_tier_name => composite_rating_tier_name) }
  ...
  let(:new_policy_enrollment) { instance_double(Openhbx::Cv2::PolicyEnrollment, 
                                                ...
                                                :rating_area => rating_area, :plan => plan_cv)}
